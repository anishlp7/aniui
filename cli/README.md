...
Built with [Uniwind](https://uniwind.dev) (recommended) or [NativeWind](https://www.nativewind.dev), [rn-primitives](https://rn-primitives.vercel.app) for accessibility, [lucide-react-native](https://lucide.dev) icons, [class-variance-authority](https://cva.style), and strict TypeScript. **100 components** + 15 pre-built screen blocks.
...
**100 components** in `cli/src/registry.ts` (tiers 1–3; the source of truth for installs and the registry):
...
- **Tier 2 — `react-native-reanimated`:** Skeleton, Drawer, Collapsible, Sidebar, Connection Banner, Typing Indicator, StreamingText (typewriter AI responses).
