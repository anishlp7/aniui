...
Built with [Uniwind](https://uniwind.dev) (recommended) or [NativeWind](https://www.nativewind.dev), [rn-primitives](https://rn-primitives.vercel.app) for accessibility, [lucide-react-native](https://lucide.dev) icons, [class-variance-authority](https://cva.style), and strict TypeScript. **148 components** + 15 pre-built screen blocks.
...
**148 components** in `cli/src/registry.ts` (tiers 1–4; the source of truth for installs and the registry):
...
- **Tier 2 — `react-native-reanimated`:** Skeleton, Drawer, Collapsible, Sidebar, Connection Banner, Typing Indicator, StreamingText (typewriter AI responses), Waveform (voice recording bars), plus a family of motion pieces (Loader, Shimmer, Marquee, Flexi Button, Save Button, Morphing Tabbar, and the carousel variants).
- **Tier 3 — extra packages:** `@rn-primitives/*`, `@gorhom/bottom-sheet`, `react-native-gesture-handler`, `expo-blur`, `expo-haptics` — e.g. Checkbox, Accordion, Command Menu, Bottom Sheet, SwipeDeck, SlideToConfirm, Tray, Unfold Menu, QR Code.
- **Tier 4 — `@shopify/react-native-skia`:** Area/Bar/Line/Pie/Radar/Radial charts, plus the gooey/morph effects family (Morph FAB, Gooey Popover, Gooey Search Tabs).
