...
const releases: Release[] = [
  {
    ...
    changes: [
      ...
      { type: "feat", text: "Single source of truth for colors: components/ui/theme-provider.tsx now exports useThemeColors(), a hook returning the current light/dark hex palette for anything that can't read a className token — TextInput caret/placeholder, Switch track/thumb, Slider fill, icon color, chart canvas fills, and more. Migrated 48 components off their own independently-hardcoded hex approximations (several of which had already drifted, e.g. #18181b vs #09090b for the same \"dark foreground\" role) onto this one hook. aniui init and aniui theme keep it in sync with whichever preset a project has chosen — picking a new preset updates both global.css and theme-provider.tsx's hex table together, and aniui add patches it in for any of these components added later. Fixed 8+ confirmed dark-mode bugs as a side effect (icons in search-bar, password-input, combobox, select, header, and others that never adapted to dark mode at all). Also unified cli/src/commands/theme.ts and init.ts's two separately hand-maintained preset objects into one cli/src/theme-presets.ts.", link: "/docs/theme-provider" },
      { type: "fix", text: "Gooey Popover: the panel was invisible in light mode (its Skia backdrop defaulted to the same white as the page background, with no border of its own for definition) and could misalign once its content wrapped to two lines (no screen-edge clamping, so a wide panel centered under a narrow trigger could overhang past the edge). Also fixed the trigger button rendering fully hidden behind its own backdrop canvas — both had an effective zIndex of 0, and the canvas being a later sibling won that tie.", link: "/docs/gooey-popover" },
    ],
  },
  {
    version: "0.6.1",
