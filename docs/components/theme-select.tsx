...
export function ThemeSelect({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <span className="h-3.5 w-3.5 rounded-full shrink-0" style={{ backgroundColor: isRandom ? undefined : hsl(displayColor.light["--primary"]), background: isRandom ? "conic-gradient(red,orange,yellow,green,cyan,blue,violet,red)" : undefined }} />
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 z-50 w-44 rounded-lg border border-border bg-background p-1 shadow-lg">
          {themeColors.map((c) => (
            <button
              key={c.name}
              onClick={() => { onChange(c.name); setOpen(false); }}
              className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-md text-xs transition-colors cursor-pointer ${
                value === c.name ? "bg-accent text-accent-foreground font-medium" : "text-foreground hover:bg-accent/50"
              }`}
            >
