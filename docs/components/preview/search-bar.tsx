...
const searchBarVariants = cva("flex items-center rounded-lg bg-muted px-3", {
  variants: {
    size: { sm: "h-10 px-2.5", md: "h-12 px-3", lg: "h-14 px-4" },
  },
...
export function PreviewSearchBar({ size = "md", className, value, onChange, icon, onClear, showCancel, onCancel, ...props }: PreviewSearchBarProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {showCancel && <button onClick={onCancel} className="text-base text-primary hover:text-primary/80 cursor-pointer">Cancel</button>}
    </div>
  );
}
