...
export function PreviewChip({ variant, size, className, selected, onClose, children, ...props }: PreviewChipProps) {
  const v = selected ? "default" : (variant ?? "outline");
  return (
    <button className={cn(chipVariants({ variant: v, size }), "font-medium", className)} {...props}>
      {children}
      {onClose && (
